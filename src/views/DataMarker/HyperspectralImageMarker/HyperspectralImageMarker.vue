        ...
        <div class="scrollbar-flex-content">
          <p v-for="item in imgArr" :key="item" class="scrollbar-demo-item">
            <div style="margin-top:5px">
              ...
              <label v-if="item.index == 1">伪彩色图像</label>
          <label v-else>通道{{item.index-1}}</label>
            </div>
          </p>
        </div>
      </el-scrollbar>
  ...
  methods: {
    getFrameNumber(videoId){
      ...
      files.forEach((filePath)=>{
        ...
        // console.log(tmp.substring(tmp.lastIndexOf("/")+1,tmp.length),videoId)
        if(tmp.substring(tmp.lastIndexOf("/")+1,tmp.length)==videoId && i<4){
          const imgPath ={
