...
export default {
  ...
  methods: {
    ...
    initMap(videoInformation, frame, option, relationVideoId) {
      this.relationVideoId = relationVideoId; // 左边标注区videoId
      ...
      this.destroyMap();
      // 发送请求，得到新的帧的画面
      this.gMap = new AILabel.Map("markerFileArea", {
        width: 655,
    ...
    addInfoIcon(feature, index) {
      ...
      gInfoMarker.events.on("click", (marker) => {
        ...
        this.$refs.trackInfo.openDialog({
          ...
          disabledRelation: false,
          relationVideoId: this.relationVideoId,
        });
      });
