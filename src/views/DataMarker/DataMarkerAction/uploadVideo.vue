...
export default {
  ...
  data() {
    return {
      ...
      visList: [],
      uploadTips:
        "已有文件将覆盖，非高光谱时只能上传1个文件且每个文件不超过1GB",
      fileAmount: 1,
  ...
  watch: {
    ...
    sensorType() {
      ...
      if (this.sensorType === "IR") {
        ...
        this.fileAmount = 1;
        this.uploadTips =
          "请选择可见光视频文件作为匹配，再上传红外视频文件(大小不超过1GB)";
  ...
  methods: {
    ...
    fileChange(file) {
      const isLt1024M = file.size / 1024 / 1024 < 1024; // 单位为MB ( B / 1024 -> KB )
      //判断大小
      console.log("----文件大小判断----");
      if (!isLt1024M) {
    ...
    fdMerge() {
      ...
      let uploadInfo = {
        shootTime: this.shootTime.Format("yyyyMMddhhmmss"),
